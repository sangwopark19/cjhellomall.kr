...
for ($i=1; $i<=11; $i++) {
    ...
    if (isset($_POST['wr_'.$i]) && settype($_POST['wr_'.$i], 'string')) {
        $$var = htmlspecialchars(trim($_POST['wr_'.$i]));
    }
}
...
$as_good = ( empty($_POST['as_good']) ) ? '0' : '1';
$sql = " UPDATE g5_board_new SET as_good='{$as_good}' WHERE bo_table='{$bo_table}' AND wr_id='{$wr_id}' ";
sql_query($sql);
