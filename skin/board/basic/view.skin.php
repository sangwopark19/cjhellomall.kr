...
<div class="view-wrap<?php echo (G5_IS_MOBILE) ? ' view-mobile font-14' : '';?><?php echo (APMS_PIM) ? ' view-modal' : '';?>">
	...
	<div class="view-btn text-right">
		<div class="btn-group" role="group">
			<?php if($bo_table == 'm0702') {?>
			<style>
				.view-btn .btn-group {text-align: center;}
				.btn-link {width: 140px;height: 40px;border: 1px solid #ddd;text-align: center;font-size: 14px;background-color: #f8f8f8;line-height: 29px;display: inline-block;float: none;}
				.btn-link:hover {background-color: #f8f8f8;color: #333;border: 1px solid #ddd;text-decoration:none;}
				.view-btn {text-align: center;}
			</style>
			<?php if ($prev_href) { ?>
				<a role="button" href="<?php echo $prev_href.$modal_query; ?>" class="btn btn-link btn-sm" title="이전글">
					<span>이전</span>
				</a>
			<?php } ?>
			<?php if ($next_href) { ?>
				<a role="button" href="<?php echo $next_href.$modal_query; ?>" class="btn btn-link btn-sm" title="다음글">
					<span>다음</span>
				</a>
			<?php } ?>
			<?php if ($update_href) { ?>
			...
			<?php } else if($bo_table == 'm0703') {?>
			<style>
				.view-btn .btn-group {text-align: center;}
				.btn-link {width: 140px;height: 40px;border: 1px solid #ddd;text-align: center;font-size: 14px;background-color: #3d3d3d;line-height: 29px;display: inline-block;float: none;color: #fff!important;}
				.btn-link:hover {background-color: #3d3d3d;color: #fff;border: 1px solid #ddd;text-decoration:none;}
				.view-btn {text-align: center;}
				.view-head {border-bottom: none;}
				.list-group-item {text-align: right;}
			</style>
				<a role="button" href="<?php echo $list_href ?>" class="btn btn-link btn-sm">
					<span> 목록</span>
				</a>
				<?php if ($update_href) { ?>
					<a role="button" href="<?php echo $update_href ?>" class="btn btn-<?php echo $btn1;?> btn-sm" title="수정"<?php echo $modal_target;?>>
