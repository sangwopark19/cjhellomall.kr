...
?>
<style>
	.at-body {max-width: 1200px;width: 100%;margin: 20px auto;}
	@media all and (max-width:991px){
		.search-box { padding-top: 77px;}
	}
</style>
...
?>
	...
	for ($i=0; $i<count($list[$idx]) && $k<$rows; $i++, $k++) {
		...
		$img = apms_wr_thumbnail($list[$idx][$i]['bo_table'], $list[$idx][$i], false, true); // 썸네일
	 ...
	 ?>
		<div class="media">
			<a href="<?php echo $list[$idx][$i]['href'] ?><?php echo $comment_href ?>">
				<div class="img-box">
					<div class="top-month"><?php echo $list[$idx][$i]['wr_5'];?></div>
					<style>
						#wr-img<?php echo $i?> {background-image: url(<?php echo $img['src'];?>)}
					</style>
					<div id="wr-img<?php echo $i?>" class="wr-img"></div>
				</div>
				<div class="txt-box">
					<div class="title-wrap">
						...
						<div class="code">
							<span class="text-muted"><?php echo $list[$idx][$i]['wr_4'];?></span>
						</div>
					</div>
					<div class="txt-content">
						<div>카드할인가 <span class="color-txt1"><span><?php echo $list[$idx][$i]['wr_1'];?></span></span></div>
						<div>헬로렌탈가 <span><?php echo $list[$idx][$i]['wr_2'];?></span></div>
					</div>
				</div>
			</a>	
		</div>
