...
// 글추출
function apms_board_rows($arr, $flag='') {
	...
	if(!$bo_table || count($board_cnt) > 1 || $except) { //복수
		...
		if( $flag == 'calnon' ) {
			$result = sql_query(" select *  $sql_common AND as_good <> 0 order by $sql_orderby $orderby1 limit $start_rows, $rows ", false);
		} else {
		...
		if( $flag == 'falcon' ) {
			$result = sql_query(" select * $sql_common AND wr_good <> 0 order by $sql_orderby $orderby2 limit $start_rows, $rows ", false);
		} else {
